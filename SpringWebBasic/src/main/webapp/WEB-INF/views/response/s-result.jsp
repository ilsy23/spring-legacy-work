...
<body>
	...
	<%-- 	
	<h2>
			<c:if test="${loginResult eq 'success'}">로그인 성공</c:if>
			<c:if test="${loginResult eq 'f-id'}">존재하지 않는 아이디입니다.</c:if>
			<c:when test="${loginResult eq 'f-pw'}">비밀번호가 일치하지 않습니다.</c:if>
	</h2> 
	--%>
	...
	<script>
		...
		*/
		...
		switch(result){
		case 'f-id':
				$h1.textContent = '존재하지 않는 아이디입니다.';
				break;
				
		case 'f-pw':
			$h1.textContent = '비밀번호가 일치하지 않습니다.';
			break;
