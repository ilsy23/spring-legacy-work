                    ...
                    <div class="col-xs-12 col-md-9 write-wrap">
	                    ...
	                    <div class="reply-content">
	                        ...
	                        <div class="reply-group">
	                              ...
	                              <input id="replyId" type="text" class="form-control" placeholder="이름">
	                              <input id="replyPw" type="password" class="form-control" placeholder="비밀번호">
	                              </div>
	                              
	                              <button id="replyRegist" type="button" class="right btn btn-info">등록하기</button>
	                        </div>
    ...
    <script>
        window.onload = function(){
            ...
            document.getElementById('moreList').onclick = () => {
                // 더보기는 누적해서 보여줘야하기 때문에 초기화하면 안됨 -> false
                getList(++page, false);
            }
            ...
            // 게시글 상세보기 화면에 처음 진입했을 시 댓글 리스트를 불러오자
            getList(1, true);

            // 댓글 목록을 가져올 함수
            // getList의 매개값으로 줄 것
            // 요청된 페이지 번호와 화면을 리셋할 것인지의 여부를 bool 타입의 reset으로 받음.
            // 비동식 방식이기 때문에 페이지가 그대로 계속 머물면서 댓글이 밑에 쌓입니다.
            // 때에 따라서 댓글을 계속 누적하는 것이 아닌, 화면을 초기화하고 새롭게 보여줘야할 때가 있다.
            // reset -> true: 페이지를 리셋해서 새롭게 그려내기, reset -> false: 누적해서 쌓기.
            function getList(pageNum, reset) {
                console.log('getList() 호출됨');
                strAdd = ''; // 새로운 내용을 작성하게 위해 변수 내용 비우기
                ...
                .then(data => {
                    ...
                    // 더보기 버튼 배치 판단
                    // 현재 페이지 번호 * 이번 요청으로 받은 댓글 수 > 전체 댓글수 -> 더보기 X
            ...
            때문에 이벤트가 등록되는 시점보다 fetch 함수의 실행이 먼저 끝날 것이라는
            보장이 없기 때문에 해당 방식은 이벤트 등록이 불가능합니다.
            이 때는 이미 실제로 존재하는 #replyList에 이빈트를 등록하고, 이벤트를 자식에게 위임하여 
            사용하는 addEventListner를 통해 처리해야 한다.
            */
           document.getElementById('replyList').addEventListener('click', e => {
            ...
            // 3. 모달 창 하나를 이용해서 상황에 맞게 수정 또는 삭제 모달을 제공해야 한다. 
            // 조건문 작성 ( 수정, 삭제에 여부에 따라 모달 디자인을 조정)
            if(e.target.classList.contains('replyModify')){
                // 수정 버튼을 눌렀으므로 수정 모달 형식으로 꾸며준다.
