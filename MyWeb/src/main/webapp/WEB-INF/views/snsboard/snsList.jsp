	...
	<script>
		...
		function regist() {
			...
			*/
			...
			fetch('${pageContent.request.contextPath}/snsboard', {
				method: 'post',
				body: formData
			})
			// 응답 데이터는 text로 전달됩니다. (uploadSuccess)
			//file input 내용을 비우고, 글 영역 비우고, class 이름이 fileDiv(미리보기)영역을 감춰주세요. -> display none
			.then(res => res.text())
			.then(data => {
				console.log(data);
				...
				for(board of list) {
					...
					`<div class="title-inner">
                        ...
                        <div class="image-inner">
                            <!-- 이미지영역 -->
                            <a href="` + board.bno + `">
                            <img data-bno="` + board.bno + `" src="${pageContext.request.contextPath}/snsboard/display/` + board.fileLoca + `/` + board.fileName + `">                            </a>
                        </div>
                        ...
                        <div class="link-inner">
                            <a id="likeBtn" href="` + board.bno + `"><img src="${pageContext.request.contextPath}/img/like1.png" width="18px" height="18px" />&nbsp좋아요</a>
        ...
        */
		const handleScroll = _.throttle(() => {
			...
			if(isFinish) {
				if(scrollPosition + windowHeigt >= height * 0.9) {
