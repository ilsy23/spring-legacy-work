...
//		System.out.println("fileName: " + fileName);
		...
		HttpHeaders headers = new HttpHeaders(); // 응다 헤더 객체 생성.
		...
		try {
			...
			// ResonseEntity 객체에 전달하고자 하는 파일을 byte[]로 변환해서 전달. (파일의 손상을 막기 위해)
			// header 내용도 같이 포함, 응답 상태 코드를 원하는 형태로 전달 가능.
	...
	public ResponseEntity<Object> delete(@PathVariable int bno, HttpSession session) {
		...
		if(!dto.getWriter().equals(id)) { // 함수 적용 대상에 null이 오지 않게!
