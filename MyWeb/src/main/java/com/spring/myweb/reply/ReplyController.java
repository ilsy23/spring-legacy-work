...
public class ReplyController {
	...
	public Map<String, Object> getList(@PathVariable int bno, @PathVariable int pageNum) {
		...
		/*
		 ...
		 2. Mapper 인터페이스에 복수의 값을 전달하기 위해 Map을 쓸지, @Param을 쓸지 결정.
		 3. ReplyMapper.xml에 sql문을 페이징 쿼리로 작성.
		 4. 클라이언트 쪽으로 DB에서 조회한 댓글 목록을 보낼 때, 페이징을 위한 댓글의 총 개수도 함꼐 보내줘야 합니다.
		 	그런데 우리는 return을 한 개만 쓸 수 있기 때문에 복수의 값을 리턴하기 위해
		 	리턴 타입을 Map으로 줄지, 객체를 디자인해서 줄지를 결정합니다.
		...
		int total = service.getTotal(bno); // 게시글에 달린 댓글의 총 개수
